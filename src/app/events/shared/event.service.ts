...
//the decorator is used when you inject a service that also injects other services as dependencies of its own.
@Injectable()

export class EventService {

    //returning a Subject which is a type of Observable.
    //therefore the return type of getEvents method is an Observable of type IEVENT array.
    constructor(private http: HttpClient){

    }

    getEvents():Observable<IEvent[]>{
        // let subject = new Subject<IEvent[]>() //subject is a type of of observable.

        // //below we are adding data in an observable stream but in an async fashion. 
        // setTimeout(() => {subject.next(EVENTS); subject.complete();},100) //after 100ms, data will be added to the stream
        // //return EVENTS
        // return subject

        //fetching the records from the server using http get method
        //will return a collection of every event present in the server's dataset
        // to utilize the error handling with rxJs, we are going to 'pipe' this get method
        //catchError -> it is a special RxJs method. we pass in the handleError private method that we created
        // it is set to IEvents array since it is going to be the return type of the getEvents method
        // when it is first called, the first parameter passed is the string which is the name of the method. It is basically used for error reporting purposes
        // it means that this method has an error during the HTTP call, then the error is reported the word 'getEvents' will get reported out. the default result is the empty array
    ...
    //specifying the method to return an event of type "IEvent"
    // getEvent(id:number):IEvent {

    //     return EVENTS.find(event => event.id === id)
    // }

    //previously the getEvent method used to send the IEvent directly i.e not in the form of an observable
    //over here instead of returning an array of events, we want to navigate to the respective event id page
    //below function will return a single event from the HTTP call
    ...
    saveEvent(event: IEvent){
        ...
        // we are telling server that we are sending JSON data
        const options = { headers: new HttpHeaders({'Content-Type': 'application/json'})}
        // event.id=999
        // event.sessions = []
        // EVENTS.push(event)

        //return the observable, so whenever somebody calls the saveEvent method they can subscribe to the observable
    ...
    searchSessions(searchTerm: string): Observable<ISession[]>{
        ...
        // //made it true so that it will emit the events asynchronously
    ...
    //priate method created to handle the Errors
    //reporting to the console about the error
...
//below, EVENTS is of the type IEVENT array.
const EVENTS: IEvent[] = [
    {
      id: 1,
...
//Having an injectable service here

//The Service uses two methods. the getEvents is used to return all the events.
//the getEvent method is used to return a single event based on an id.

//Resolve allows you to prefetch the necessary data for a component or to do other checks prior to loading a component.
//observables are streams of data. they are arrays where the data arrives on time.

//Note: using observables means the data of the component will load after the time specified has passed out.
